...
class MediaProcessor:
    ...
    def fullprocess(self, inputfile, mediatype, reportProgress=False, original=None, info=None, tmdbid=None, tvdbid=None, imdbid=None, season=None, episode=None, language=None, tagdata=None):
        try:
            ...
            if info:
                ...
                output = self.process(
                    inputfile, original=original, info=info, tagdata=tagdata)

                if output:
                    if not language:
                        language = self.settings.taglanguage or self.getDefaultAudioLanguage(
                            output["options"]) or None
                    self.log.debug("Tag language setting is %s, using language %s for tagging." % (
                        self.settings.taglanguage or None, language))
                    # Tag with metadata
                    tagfailed = False
                    try:
                        tag = Metadata(mediatype, tvdbid=tvdbid, tmdbid=tmdbid, imdbid=imdbid,
                                       season=season, episode=episode, original=original, language=language)
                        tmdbid = tag.tmdbid
                        if self.settings.tagfile:
                            self.log.info("Tagging %s with TMDB ID %s." %
                                          (inputfile, tag.tmdbid))
                            tag.writeTags(output['output'], self.converter, self.settings.artwork,
                                          self.settings.thumbnail, output['x'], output['y'])
                    except:
                    ...
                    # Reverse Ouput
                    output['output'] = self.restoreFromOutput(
                    ...
                    if self.settings.postprocess:
                        postprocessor = PostProcessor(
                            output_files, self.log, wait=self.settings.waitpostprocess)
                        postprocessor.setEnv(
                    ...
                    if self.settings.Plex.get('refresh', False):
                        try:
                            plex.refreshPlex(
                                self.settings, mediatype, self.log)
                        except:
    ...
    def process(self, inputfile, reportProgress=False, original=None, info=None, progressOutput=None, tagdata=None):
        ...
        if info:
            try:
                options, preopts, postopts, ripsubopts, downloaded_subs = self.generateOptions(
                    inputfile, info=info, original=original, tagdata=tagdata)
            except:
                self.log.exception(
            ...
            if self.canBypassConvert(inputfile, info, options):
                ...
                if not options:
                    self.log.error(
                        "Error converting, inputfile %s had a valid extension but returned no data. Either the file does not exist, was unreadable, or was an incorrect format." % inputfile)
                    return None

                try:
                    self.log.info("Output Data")
                    self.log.info(json.dumps(
                        options, sort_keys=False, indent=4))
                    self.log.info("Preopts")
                    self.log.info(json.dumps(
                        preopts, sort_keys=False, indent=4))
                    self.log.info("Postopts")
                    self.log.info(json.dumps(
                        postopts, sort_keys=False, indent=4))
                    if not self.settings.embedsubs:
                        self.log.info("Subtitle Extracts")
                        self.log.info(json.dumps(
                            ripsubopts, sort_keys=False, indent=4))
                    if self.settings.downloadsubs:
                        self.log.info("Downloaded Subtitles")
                        self.log.info(json.dumps(downloaded_subs,
                ...
                try:
                    outputfile, inputfile = self.convert(
                        options, preopts, postopts, reportProgress, progressOutput)
                except:
                    self.log.exception(
            ...
            if not outputfile:
                self.log.debug(
                    "Error converting, no outputfile generated for inputfile %s." % inputfile)
                return None

            self.log.debug("%s created from %s successfully." %
                           (outputfile, inputfile))
                ...
                if self.settings.output_dir:
                    try:
                        outputfile = os.path.join(
                            self.settings.output_dir, os.path.split(inputfile)[1])
            ...
            if delete:
                ...
                for subfile in self.deletesubs:
                    self.log.debug(
                        "Attempting to remove subtitle %s." % subfile)
                    if self.removeFile(subfile):
                        self.log.debug("Subtitle %s deleted." % subfile)
                    else:
                        self.log.debug(
                            "Unable to delete subtitle %s." % subfile)
                self.deletesubs = set()
    ...
    def cleanDispositions(self, info):
        for stream in info.streams:
            for dispo in self.settings.sanitize_disposition:
                self.log.debug(
                    "Setting %s to False for stream %d [sanitize-disposition]." % (dispo, stream.index))
    ...
    def isValidSource(self, inputfile):
        try:
            ...
            if extension in self.settings.ignored_extensions:
                self.log.debug(
                    "Invalid source, extension is blacklisted [ignored-extensions].")
                return None
            if self.settings.minimum_size > 0 and os.path.getsize(inputfile) < (self.settings.minimum_size * 1000000):
                self.log.debug(
                    "Invalid source, below minimum size threshold [minimum-size].")
                return None
            ...
            if validation:
                try:
                    if not validation(self, info, inputfile):
                        self.log.debug(
                            "Failed custom validation check, file is not valid.")
                        return None
                except:
            ...
            self.log.exception(
                "isValidSource unexpectedly threw an exception, returning None.")
            return None
    ...
    def isValidSubtitleSource(self, inputfile):
        try:
            ...
            self.log.exception(
                "isValidSubtitleSource unexpectedly threw an exception, returning None.")
            return None

    def getDefaultAudioLanguage(self, options):
        for a in options.get("audio", []):
            if "+default" in a.get("disposition", "").lower():
                self.log.debug("Default audio language is %s." %
                               a.get("language"))
    ...
    def estimateVideoBitrate(self, info):
        try:
            ...
            self.log.debug("Estimated video bitrate is %s." %
    ...
    def jsonDump(self, inputfile, original=None):
        ...
        dump["output"], dump["preopts"], dump["postopts"], dump["ripsubopts"], dump["downloadedsubs"] = self.generateOptions(
            inputfile, info=info, original=original)
        if self.canBypassConvert(inputfile, info, dump["output"]):
            ...
            outputfile, output_extension = self.getOutputFile(
                input_dir, filename, input_extension)
            cmds = self.converter.ffmpeg.generateCommands(
                outputfile, parsed, dump["preopts"], dump["postopts"])
            dump["ffmpeg_commands"] = []
            dump["ffmpeg_commands"].append(" ".join(
                "\"%s\"" % item if " " in item and "\"" not in item else item for item in cmds))
            for suboptions in dump["ripsubopts"]:
                ...
                suboutputfile = self.getSubOutputFileFromOptions(
                    inputfile, suboptions, extension)
                subcmds = self.converter.ffmpeg.generateCommands(
                    suboutputfile, subparsed)
                dump["ffmpeg_commands"].append(
                    " ".join(str(item) for item in subcmds))
            for sub in dump["downloadedsubs"]:
                self.log.debug(
                    "Cleaning up downloaded sub %s which was only used to simulate options." % (sub))
                self.removeFile(sub)
    ...
    def safeLanguage(self, info):
        ...
        for a in info.audio:
            a.metadata['language'] = getAlpha3TCode(
                a.metadata.get('language'), self.settings.adl)
        ...
        if overrideLang and self.settings.allow_language_relax:
            awl = []
            self.log.info(
                "No audio streams detected in any appropriate language, relaxing restrictions [allow-audio-language-relax].")

        # Prep subtitle streams by cleaning up languages and setting SDL
        for s in info.subtitle:
            s.metadata['language'] = getAlpha3TCode(
                s.metadata.get('language'), self.settings.sdl)
        return awl, swl
    ...
    def titleDispositionCheck(self, info):
        for stream in info.streams:
            ...
            if 'comment' in title:
                self.log.debug(
                    "Found comment in stream title, setting comment disposition to True.")
                stream.disposition['comment'] = True
            if 'hearing' in title:
                self.log.debug(
                    "Found hearing in stream title, setting hearing_impaired disposition to True.")
                stream.disposition['hearing_impaired'] = True
            if 'visual' in title:
                self.log.debug(
                    "Found visual in stream title, setting visual_impaired disposition to True.")
                stream.disposition['visual_impaired'] = True
            if 'forced' in title:
                self.log.debug(
                    "Found foced in stream title, setting forced disposition to True.")
    ...
    def generateOptions(self, inputfile, info=None, original=None, tagdata=None):
        ...
        if not info:
            self.log.error("FFPROBE returned no value for inputfile %s (exists: %s), either the file does not exist or is not a format FFPROBE can read." % (
                inputfile, os.path.exists(inputfile)))
        ...
        vcodecs = self.settings.hdr.get('codec', []) if vHDR and len(
            self.settings.hdr.get('codec', [])) > 0 else self.settings.vcodec
        ...
        try:
            if blockVideoCopy and blockVideoCopy(self, info.video, inputfile):
                self.log.exception(
                    "Custom video stream copy check is preventing copying the stream.")
        ...
        self.log.debug("Using video bitrate ratio of %f, which results in %f changing to %f." % (
            vbitrate_ratio, vbitrate_estimate, vbitrate))
        if self.settings.vmaxbitrate and vbitrate > self.settings.vmaxbitrate:
            self.log.debug(
                "Overriding video bitrate. Codec cannot be copied because video bitrate is too high [video-max-bitrate].")
        ...
        if self.settings.vwidth and self.settings.vwidth < info.video.video_width:
            self.log.debug(
                "Video width is over the max width, it will be downsampled. Video stream can no longer be copied [video-max-width].")
        ...
        if self.settings.video_level and info.video.video_level and (info.video.video_level > self.settings.video_level):
            self.log.debug(
                "Video level %0.1f. Codec cannot be copied because video level is too high [video-max-level]." % (info.video.video_level))
        ...
        if vHDR and len(self.settings.hdr.get('profile')) > 0:
            if info.video.profile in self.settings.hdr.get('profile'):
                ...
                self.log.debug(
                    "Overriding video profile. Codec cannot be copied because profile is not approved [hdr-profile].")
            ...
            if len(self.settings.vprofile) > 0:
                if info.video.profile in self.settings.vprofile:
                    ...
                    vprofile = self.settings.vprofile[0] if len(
                        self.settings.vprofile) > 0 else None
                    self.log.debug(
            ...
            for profile in self.settings.vcrf_profiles:
                try:
                    if profile['source_bitrate'] < vbitrate_estimate:
                        ...
                        self.log.info("Acceptable profile match found for VBR %s using CRF %d, maxrate %s, bufsize %s." % (
                            vbitrate_estimate, vcrf, vmaxrate, vbufsize))
                        break
                except:
                    self.log.exception(
                        "Error setting VCRF profile information.")

        vfilter = self.settings.hdr.get(
            'filter') or None if vHDR else self.settings.vfilter or None
        if vHDR and self.settings.hdr.get('filter') and self.settings.hdr.get('forcefilter'):
            self.log.debug(
                "Video HDR force filter is enabled. Video stream can no longer be copied [hdr-force-filter].")
            vdebug = vdebug + ".hdr-force-filter"
            vcodec = vcodecs[0]
        elif not vHDR and vfilter and self.settings.vforcefilter:
            self.log.debug(
                "Video force filter is enabled. Video stream can no longer be copied [video-force-filter].")
            vfilter = self.settings.vfilter
            vcodec = vcodecs[0]
            vdebug = vdebug + ".force-filter"

        vpreset = self.settings.hdr.get(
            'preset') or None if vHDR else self.settings.preset or None
        ...
        vframedata = self.normalizeFramedata(
            info.video.framedata, vHDR) if self.settings.dynamic_params else None
        ...
        if vHDR and len(self.settings.hdr.get('pix_fmt')) > 0:
            if info.video.pix_fmt in self.settings.hdr.get('pix_fmt'):
                ...
                self.log.debug(
                    "Overriding video pix_fmt. Codec cannot be copied because pix_fmt is not approved [hdr-pix-fmt].")
            ...
            if info.video.pix_fmt in self.settings.pix_fmt:
                ...
                vpix_fmt = self.settings.pix_fmt[0] if len(
                    self.settings.pix_fmt) else None
                self.log.debug(
        ...
        self.log.info("Creating %s video stream from source stream %d." %
                      (vcodec, info.video.index))
        ...
        for a in audio_streams:
            self.log.info("Audio detected for stream %s - %s %s %d channel." %
                          (a.index, a.codec, a.metadata['language'], a.audio_channels))
            ...
            if a.codec == 'truehd' and self.settings.output_extension in self.settings.ignore_truehd:
                if len([x for x in info.audio if x.audio_channels == a.audio_channels and x.metadata['language'] == a.metadata['language']]) > 1:
                    self.log.info(
                        "Skipping trueHD stream %s as typically the 2nd audio stream is the AC3 core of the truehd stream [audio-ignore-truehd]." % a.index)
                    continue
                else:
                    self.log.info(
                        "TrueHD stream detected but no other comparable audio streams in source, cannot skip stream %s [audio-ignore-truehd]." % a.index)
            ...
            if self.validLanguage(a.metadata['language'], awl, blocked_audio_languages) and self.validDisposition(a.metadata['language'], a.dispostr, self.settings.ignored_audio_dispositions, self.settings.unique_audio_dispositions, blocked_audio_dispositions):
                ...
                if ua and a.audio_channels > 2:
                    if self.settings.ua_bitrate == 0:
                        self.log.warning(
                            "Universal audio channel bitrate must be greater than 0, defaulting to %d [universal-audio-channel-bitrate]." % self.default_channel_bitrate)
                        self.settings.ua_bitrate = self.default_channel_bitrate
                    ua_bitrate = (self.default_channel_bitrate * 2) if (self.settings.ua_bitrate * 2) > (
                        self.default_channel_bitrate * 2) else (self.settings.ua_bitrate * 2)
                    ua_disposition = a.dispostr
                    ua_filter = self.settings.ua_filter or None

                    # Custom channel based filters
                    ua_afilterchannel = self.settings.afilterchannels.get(
                        a.audio_channels, {}).get(2)
                    if ua_afilterchannel:
                        ua_filter = "%s,%s" % (
                            ua_filter, ua_afilterchannel) if ua_filter else ua_afilterchannel
                        self.log.debug("Found an audio filter for converting from %d channels to %d channels. Applying filter %s to UA." % (
                            (a.audio_channels, 2, ua_afilterchannel)))

                    # Bitrate calculations/overrides
                    if self.settings.ua_bitrate == 0:
                        self.log.debug(
                            "Attempting to set universal audio stream bitrate based on source stream bitrate.")
                        try:
                            ua_bitrate = ((a.bitrate / 1000) /
                                          a.audio_channels) * 2
                        except:
                            self.log.warning("Unable to determine universal audio bitrate from source stream %s, defaulting to %d per channel." % (
                                a.index, self.default_channel_bitrate))
                ...
                if ua and a.audio_channels <= 2:
                    ...
                    abitrate = (a.audio_channels * self.default_channel_bitrate) if (a.audio_channels * self.settings.ua_bitrate) > (
                        a.audio_channels * self.default_channel_bitrate) else (a.audio_channels * self.settings.ua_bitrate)
                    ...
                    try:
                        if blockAudioCopy and blockAudioCopy(self, a, inputfile):
                            self.log.exception(
                                "Custom audio stream copy check is preventing copying the stream.")
                            adebug = adebug + ".custom"
                            acodec = self.settings.ua[0]
                    except:
                        self.log.exception(
                            "Custom audio stream copy check error.")

                    # UA Filters
                    afilter = self.settings.ua_filter or None
                    if afilter and self.settings.ua_forcefilter:
                        self.log.debug(
                            "Unable to copy codec because an universal audio filter is set [universal-audio-force-filter].")
                        acodec = self.settings.ua[0]
                    ...
                    if len(self.settings.audio_samplerates) > 0 and a.audio_samplerate not in self.settings.audio_samplerates:
                        self.log.debug(
                            "Unable to copy codec because audio sample rate %d is not approved [audio-sample-rates]." % (a.audio_samplerate))
                    ...
                    acodec = 'copy' if a.codec in self.settings.acodec else self.settings.acodec[
                        0]
                    # Audio channel adjustments
                    if self.settings.maxchannels and a.audio_channels > self.settings.maxchannels:
                        self.log.debug(
                            "Audio source exceeds maximum channels, can not be copied. Settings channels to %d [audio-max-channels]." % self.settings.maxchannels)
                    ...
                    if afilter and self.settings.aforcefilter:
                        self.log.debug(
                            "Unable to copy codec because an audio filter is set [audio-force-filter].")
                        acodec = self.settings.acodec[0]
                        adebug = adebug + ".audio-force-filter"

                    # Custom channel based filters
                    afilterchannel = self.settings.afilterchannels.get(
                        a.audio_channels, {}).get(audio_channels)
                    if afilterchannel:
                        afilter = "%s,%s" % (
                            afilter, afilterchannel) if afilter else afilterchannel
                        self.log.debug("Found an audio filter for converting from %d channels to %d channels. Applying filter %s." % (
                            a.audio_channels, audio_channels, afilterchannel))
                ...
                if self.settings.abitrate == 0:
                    ...
                    try:
                        ...
                        self.log.warning("Unable to determine audio bitrate from source stream %s, defaulting to %d per channel." % (
                            a.index, self.default_channel_bitrate))
                        abitrate = audio_channels * self.default_channel_bitrate
                if self.settings.amaxbitrate and abitrate > self.settings.amaxbitrate:
                    self.log.debug("Calculated bitrate of %d exceeds maximum bitrate %d, setting to max value [audio-max-bitrate]." % (
                ...
                self.log.info(
                    "Creating %s audio stream from source stream %d." % (acodec, a.index))
                ...
                if uadata:
                    self.log.info(
                        "Creating %s audio stream from source audio stream %d [universal-audio]." % (uadata.get('codec'), a.index))
                    uaposition = len(
                        audio_settings) if self.settings.ua_last else aposition
                    audio_settings.insert(uaposition, uadata)
                    aposition = aposition if self.settings.ua_last else (
                        aposition + 1)
                ...
                if self.settings.audio_copyoriginal and acodec != 'copy' and not (a.codec == 'truehd' and self.settings.output_extension in self.settings.ignore_truehd):
                    self.log.info(
                        "Copying audio stream from source stream %d format %s [audio-copy-original]." % (a.index, a.codec))
                    aposition = aposition if self.settings.audio_copyoriginal_before else (
                        aposition + 1)
                ...
                if self.settings.audio_first_language_stream:
                    ...
                    self.log.debug(
                        "Blocking further %s audio streams to prevent multiple streams of the same language [audio-first-stream-of-language]." % a.metadata['language'])
        ...
        if not self.settings.ignore_embedded_subs:
            for s in info.subtitle:
                try:
                    ...
                    self.log.error(
                        "Unknown error occurred while trying to determine if subtitle is text or image based. Probably corrupt, skipping.")
                    continue
                self.log.info("%s-based subtitle detected for stream %s - %s %s." % (
                    "Image" if image_based else "Text", s.index, s.codec, s.metadata['language']))
                ...
                if image_based and self.settings.embedimgsubs and self.settings.scodec_image and len(self.settings.scodec_image) > 0:
                    scodec = 'copy' if s.codec in self.settings.scodec_image else self.settings.scodec_image[
                        0]
                elif not image_based and self.settings.embedsubs and self.settings.scodec and len(self.settings.scodec) > 0:
                    scodec = 'copy' if s.codec in self.settings.scodec else self.settings.scodec[
                        0]
                ...
                if scodec:
                    ...
                    if self.validLanguage(s.metadata['language'], swl, blocked_subtitle_languages) and self.validDisposition(s.metadata['language'], sdisposition, self.settings.ignored_subtitle_dispositions, self.settings.unique_subtitle_dispositions, blocked_subtitle_dispositions):
                        self.log.info(
                            "Creating %s subtitle stream from source stream %d." % (scodec, s.index))
                        ...
                        if self.settings.sub_first_language_stream:
                            blocked_subtitle_languages.append(
                                s.metadata['language'])
                else:
                    if self.validLanguage(s.metadata['language'], swl, blocked_subtitle_languages) and self.validDisposition(s.metadata['language'], sdisposition, self.settings.ignored_subtitle_dispositions, self.settings.unique_subtitle_dispositions, blocked_subtitle_dispositions):
        ...
        downloaded_subs = []
        try:
            downloaded_subs = self.downloadSubtitles(
                inputfile, info.subtitle, swl, original, tagdata)
        except:
        ...
        if not self.settings.embedonlyinternalsubs:
            ...
            for external_sub in valid_external_subs:
                try:
                    image_based = self.isImageBasedSubtitle(
                        external_sub.path, 0)
                except:
                    self.log.error(
                        "Unknown error occurred while trying to determine if subtitle is text or image based. Probably corrupt, skipping.")
                ...
                if not scodec:
                    self.log.info("Skipping external subtitle file %s, no appropriate codecs found or embed disabled." %
                                  os.path.basename(external_sub.path))
                    continue
                ...
                if self.validLanguage(external_sub.subtitle[0].metadata['language'], swl, blocked_subtitle_languages) and self.validDisposition(external_sub.subtitle[0].metadata['language'], sdisposition, self.settings.ignored_subtitle_dispositions, self.settings.unique_subtitle_dispositions, blocked_subtitle_dispositions):
                    ...
                    self.log.info("Creating %s subtitle stream by importing %s-based %s [embed-subs]." % (
                        scodec, "Image" if image_based else "Text", os.path.basename(external_sub.path)))
                    subtitle_settings.append({
        ...
        # Burn subtitles
        try:
            vfilter = self.burnSubtitleFilter(
                inputfile, info.subtitle, swl, valid_external_subs)
        except:
            vfilter = None
            self.log.exception(
                "Encountered an error while trying to determine which subtitle stream for subtitle burn [burn-subtitle].")
        if vfilter:
            self.log.debug(
        ...
        if options.get('format') in ['mp4']:
            for a in options['audio']:
                if info.streams[a.get('map')].codec == 'truehd' and a.get('codec') == 'copy':
                    self.log.debug(
                        "Adding experimental flag for mp4 with trueHD as a trueHD stream is being copied.")
            ...
            try:
                ...
                for k in self.settings.hwdevices:
                    if k in vcodec:
                        ...
                        self.log.debug(
                            "Found a matching device %s for encoder %s [hwdevices]." % (match, vcodec))
                        if not device:
                            self.log.debug(
                                "No device was set by the decoder, setting device to %s for encoder %s [hwdevices]." % (match, vcodec))
                            preopts.extend(
                                ['-init_hw_device', '%s=sma:%s' % (k, match)])
                            options['video']['device'] = "sma"
                        elif device == match:
                            self.log.debug(
                                "Device was already set by the decoder, using same device %s for encoder %s [hwdevices]." % (device, vcodec))
                            options['video']['device'] = "sma"
                        else:
                            self.log.debug(
                                "Device was already set by the decoder but does not match encoder, using secondary device %s for encoder %s [hwdevices]." % (match, vcodec))
                            preopts.extend(
                                ['-init_hw_device', '%s=sma2:%s' % (k, match)])
                            options['video']['device'] = "sma2"
                            options['video']['decode_device'] = "sma"
                        break
            except:
                self.log.exception(
                    "Error when trying to determine hardware acceleration support.")
        ...
        for hwaccel in self.settings.hwaccels:
            if hwaccel in hwaccels:
                ...
                if device:
                    ...
                    opts.extend(['-init_hw_device', '%s=sma:%s' %
                                 (hwaccel, device)])
                    opts.extend(['-hwaccel_device', 'sma'])

                self.log.info(
                    "%s hwaccel is supported by this ffmpeg build and will be used [hwaccels]." % hwaccel)
                opts.extend(['-hwaccel', hwaccel])
                if self.settings.hwoutputfmt.get(hwaccel):
                    opts.extend(['-hwaccel_output_format',
                                 self.settings.hwoutputfmt[hwaccel]])

                # If there's a decoder for this acceleration platform, also use it
                decoder = self.converter.ffmpeg.hwaccel_decoder(
                    video_codec, hwaccel)
                self.log.debug("Decoder: %s." % decoder)
                if (decoder in codecs[video_codec]['decoders'] and decoder in self.settings.hwaccel_decoders):
                    self.log.info(
                        "%s decoder is also supported by this ffmpeg build and will also be used [hwaccel-decoders]." % decoder)
    ...
    def setDefaultAudioStream(self, audio_settings):
        if len(audio_settings) > 0:
            audio_streams = sorted(audio_settings, key=lambda x: x.get(
                'channels', 1), reverse=self.settings.default_more_channels)
            audio_streams.sort(
                key=lambda x: '+comment' in (x.get('disposition') or ''))
            preferred_language_audio_streams = [x for x in audio_streams if x.get(
                'language') == self.settings.adl] if self.settings.adl else audio_streams
            default_stream = audio_streams[0]
            default_streams = [
                x for x in audio_streams if '+default' in (x.get('disposition') or '')]
            default_preferred_language_streams = [x for x in default_streams if x.get(
                'language') == self.settings.adl] if self.settings.adl else default_streams
            default_streams_not_in_preferred_language = [
                x for x in default_streams if x not in default_preferred_language_streams]

            self.log.debug("%d total audio streams with %d set to default disposition. %d defaults in your preferred language (%s), %d in other languages." % (
            ...
            if len(default_preferred_language_streams) < 1:
                try:
                    potential_streams = preferred_language_audio_streams if len(
                        preferred_language_audio_streams) > 0 else default_streams
                    default_stream = potential_streams[0] if len(
                        potential_streams) > 0 else audio_streams[0]
                except:
                    self.log.exception(
                        "Error setting default stream in preferred language.")
                ...
                try:
                    for remove in default_preferred_language_streams[1:]:
                        if remove.get('disposition'):
                            remove['disposition'] = remove.get(
                                'disposition').replace('+default', '-default')
                    self.log.debug("%d streams in preferred language cleared of default disposition flag from preferred language." % (
                        len(default_preferred_language_streams) - 1))
                except:
                    self.log.exception(
                        "Error in removing default disposition flag from extra audio streams, multiple streams may be set as default.")
            else:
                self.log.debug(
                    "Default audio stream already inherited from source material, will not override to audio-language-default.")
                default_stream = default_preferred_language_streams[0]

            default_streams_not_in_preferred_language = [
                x for x in default_streams_not_in_preferred_language if x != default_stream]
            if len(default_streams_not_in_preferred_language) > 0:
                self.log.debug("Cleaning up default disposition settings from not preferred languages. %d streams will have default flag removed." % (
                    len(default_streams_not_in_preferred_language)))
                for remove in default_streams_not_in_preferred_language:
                    if remove.get('disposition'):
                        remove['disposition'] = remove.get(
                            'disposition').replace('+default', '-default')
            if default_stream.get('disposition'):
                default_stream['disposition'] = default_stream.get(
                    'disposition').replace('-default', '+default')
            ...
            self.log.info("Default audio stream set to %s %s %s channel stream [default-more-channels: %s]." % (
                default_stream['language'], default_stream['codec'], default_stream['channels'], self.settings.default_more_channels))
        else:
            self.log.debug(
                "Audio output is empty, unable to set default audio streams.")
    ...
    def setDefaultSubtitleStream(self, subtitle_settings):
        if len(subtitle_settings) > 0 and self.settings.sdl:
            if len([x for x in subtitle_settings if '+default' in (x.get('disposition') or '')]) < 1:
                default_stream = [x for x in subtitle_settings if x.get(
                    'language') == self.settings.sdl][0]
                ...
                self.log.debug(
                    "Default subtitle stream already inherited from source material, will not override to subtitle-language-default.")
        else:
            self.log.debug(
        ...
        if self.settings.sort_streams:
            ...
            if len(streams) > 0:
                if isinstance(streams[0], dict):
                    if codecs:
                        streams.sort(key=lambda x: languages.index(
                            x.get('codec')) if x.get('codec') in codecs else 999)
                    streams.sort(key=lambda x: x.get('channels', 999),
                                 reverse=self.settings.prefer_more_channels)
                    if languages:
                        streams.sort(key=lambda x: languages.index(
                            x.get('language')) if x.get('language') in languages else 999)
                else:
                    if codecs:
                        streams.sort(key=lambda x: codecs.index(
                            x.codec) if x.codec in codecs else 999)
                    streams.sort(key=lambda x: x.audio_channels,
                                 reverse=self.settings.prefer_more_channels)
                    if languages:
                        streams.sort(key=lambda x: languages.index(x.metadata.get(
                            'language')) if x.metadata.get('language') in languages else 999)
    ...
    def validDisposition(self, language, disposition, ignored, unique, existing, append=True):
        ...
        for dispo in truedispositions:
            if dispo in ignored:
                self.log.debug(
                    "Ignoring stream because disposition %s is on the ignore list." % (dispo))
                return False
        if unique:
            search = "%s.%s" % (language, disposition)
            if search in existing:
                self.log.debug(
                    "Invalid disposition, stream fitting this disposition profile already exists, ignoring.")
                return False
            if append:
                self.log.debug(
                    "Valid disposition, adding %s to the ignored list." % (search))
    ...
    def burnSubtitleFilter(self, inputfile, subtitle_streams, swl, valid_external_subs=None):
        if self.settings.burn_subtitles:
            filtered_subtitle_streams = [x for x in subtitle_streams if self.validLanguage(
                x.metadata.get('language'), swl)]
            filtered_subtitle_streams = sorted(filtered_subtitle_streams, key=lambda x: swl.index(
                x.metadata.get('language')) if x.metadata.get('language') in swl else 999)
            sub_candidates = []
            if len(filtered_subtitle_streams) > 0:
                ...
                sub_candidates = [x for x in filtered_subtitle_streams if self.checkDisposition(
                    self.settings.burn_dispositions, x.disposition)]
                ...
                if len(sub_candidates) > 0:
                    self.log.debug(
                        "Found %d potential sources from the included subs for burning [burn-subtitle]." % len(sub_candidates))
                    burn_sub = sub_candidates[0]
                    relative_index = burn_sub.index - first_index
                    self.log.info("Burning subtitle %d %s into video steram [burn-subtitles]." % (
                        burn_sub.index, burn_sub.metadata['language']))
                    self.log.debug(
                        "Video codec cannot be copied because valid burn subtitle was found [burn-subtitle: %s]." % (self.settings.burn_subtitles))
            ...
            if self.settings.embedsubs:
                ...
                valid_external_subs = valid_external_subs if valid_external_subs else self.scanForExternalSubs(
                    inputfile, swl)

                # Filter subtitles to be burned based on setting
                sub_candidates = [x for x in valid_external_subs if self.checkDisposition(
                    self.settings.burn_dispositions, x.subtitle[0].disposition)]
                # Filter out image based subtitles (until we can find a method to get this to work)
                for x in sub_candidates[:]:
                    try:
                ...
                if len(sub_candidates) > 0:
                    burn_sub = sub_candidates[0]
                    self.log.info("Burning external subtitle %s %s into video steram [burn-subtitles, embed-subs]." % (
                        os.path.basename(burn_sub.path), burn_sub.subtitle[0].metadata['language']))
        ...
        for dirName, subdirList, fileList in os.walk(input_dir):
            for fname in fileList:
                ...
                if fname.startswith(filename):  # filename in fname:
                    valid_external_sub = self.isValidSubtitleSource(
                    ...
                    if valid_external_sub:
                        ...
                        if self.validLanguage(lang, swl):
                            self.log.debug(
                                "External %s subtitle file detected %s." % (lang, fname))
                            for dispo in BaseCodec.DISPOSITIONS:
                                valid_external_sub.subtitle[0].disposition[dispo] = (
                                    "." + dispo) in fname
            ...
            break
        self.log.info("Scanned for external subtitles and found %d results in your approved languages." % (
            len(valid_external_subs)))
        valid_external_subs.sort(key=lambda x: swl.index(
            x.subtitle[0].metadata['language']) if x.subtitle[0].metadata['language'] in swl else 999)
        ...
        if self.settings.downloadsubs:
            ...
            for alpha3 in swl:
                try:
                    ...
                    self.log.exception(
                        "Unable to add language for download with subliminal.")
            ...
            if len(languages) < 1:
                self.log.error(
                    "No valid subtitle download languages detected, subtitles will not be downloaded.")
                return []
            ...
            try:
                options = None
                video = MediaProcessor.custom_scan_video(
                    os.path.abspath(inputfile), tagdata)

                if self.settings.ignore_embedded_subs:
                    video.subtitle_languages = set()
                else:
                    video.subtitle_languages = set(
                        [Language(x.metadata['language']) for x in existing_subtitle_streams])

                if tagdata:
                    self.log.debug(
                        "Refining subliminal search using included metadata")
                    ...
                    if tagdata.mediatype == MediaType.Movie and isinstance(video, subliminal.Movie):
                        subliminal.refine(
                            video, title=tagdata.title, year=tagdate, imdb_id=tagdata.imdbid)
                        video.title = tagdata.title or video.title
                    elif tagdata.mediatype == MediaType.TV and isinstance(video, subliminal.Episode):
                        subliminal.refine(video, series=tagdata.showname, year=tagdate,
                                          series_imdb_id=tagdata.imdbid, series_tvdb_id=tagdata.tvdbid, title=tagdata.title)
                ...
                if original:
                    try:
                        self.log.debug(
                            "Found original filename, adding data from %s." % original)
                        og = guessit(original)
                        self.log.debug("Source %s, release group %s, resolution %s, streaming service %s." % (
                            og.get('source'), og.get('release_group'), og.get('screen_size'), og.get('streaming_service')))
                        video.source = og.get('source') or video.source
                        video.release_group = og.get(
                            'release_group') or video.release_group
                        video.resolution = og.get(
                            'screen_size') or video.resolution
                        video.streaming_service = og.get(
                            'streaming_service') or video.streaming_service
                    except:
                        self.log.exception(
                            "Error importing original file data for subliminal, will attempt to proceed.")
                ...
                paths = [subliminal.subtitle.get_subtitle_path(
                    video.name, x.language) for x in saves]
                for path in paths:
    ...
    def setPermissions(self, path):
        try:
            os.chmod(path, self.settings.permissions.get(
                'chmod', int('0664', 8)))
            if os.name != 'nt':
                os.chown(path, self.settings.permissions.get(
                    'uid', -1), self.settings.permissions.get('gid', -1))
        except:
    ...
    def restoreFromOutput(self, inputfile, outputfile):
        if self.settings.output_dir and outputfile.startswith(self.settings.output_dir):
            input_dir, filename, input_extension = self.parseFile(inputfile)
            newoutputfile, _ = self.getOutputFile(
                input_dir, filename, input_extension, ignore_output_dir=True)
            self.log.info("Output file is in output_dir %s, moving back to original directory %s." % (
                self.settings.output_dir, outputfile))
    ...
    def getSubExtensionFromCodec(self, codec):
        try:
            ...
            self.log.info(
                "Wasn't able to determine subtitle file extension, defaulting to codec %s." % codec)
            return codec
    ...
    def getSubOutputFile(self, inputfile, language, disposition, extension):
        ...
        outputfile = os.path.join(
            output_dir, filename + "." + language + dispo + "." + extension)

        i = 2
        while os.path.isfile(outputfile):
            self.log.debug("%s exists, appending %s to filename." %
                           (outputfile, i))
            outputfile = os.path.join(
                output_dir, filename + "." + language + dispo + "." + str(i) + "." + extension)
            i += 1
        return outputfile
    ...
    def ripSubs(self, inputfile, ripsubopts):
        ...
        for options in ripsubopts:
            ...
            outputfile = self.getSubOutputFileFromOptions(
                inputfile, options, extension)

            try:
                self.log.info("Ripping %s subtitle from source stream %s into external file." % (
                    options["language"], options['index']))
                conv = self.converter.convert(
                    outputfile, options, timeout=None)
                _, cmds = next(conv)
                ...
                self.log.error("Unable to create external %s subtitle file for stream %s, may be an incompatible format." % (
                    extension, options['index']))
                self.removeFile(outputfile)
                continue
            except:
                self.log.exception(
                    "Unable to create external subtitle file for stream %s." % (options['index']))
    ...
    def getOutputFile(self, input_dir, filename, input_extension, temp_extension=None, ignore_output_dir=False, number=0):
        ...
        try:
            outputfile = os.path.join(output_dir.decode(sys.getfilesystemencoding()), filename.decode(
                sys.getfilesystemencoding()) + counter + "." + output_extension).encode(sys.getfilesystemencoding())
        except:
            outputfile = os.path.join(
                output_dir, filename + counter + "." + output_extension)
    ...
    def hasValidFrameData(self, framedata):
        try:
            if 'side_data_list' in framedata:
                types = [x['side_data_type']
                         for x in framedata['side_data_list'] if 'side_data_type' in x]
            ...
            if 'side_data_list' in framedata:
                for side_data in framedata['side_data_list']:
                    if side_data.get('side_data_type') == 'Mastering display metadata':
                        side_data['red_x'] = self.parseAndNormalize(
                            side_data.get('red_x'), 50000)
                        side_data['red_y'] = self.parseAndNormalize(
                            side_data.get('red_y'), 50000)
                        side_data['green_x'] = self.parseAndNormalize(
                            side_data.get('green_x'), 50000)
                        side_data['green_y'] = self.parseAndNormalize(
                            side_data.get('green_y'), 50000)
                        side_data['blue_x'] = self.parseAndNormalize(
                            side_data.get('blue_x'), 50000)
                        side_data['blue_y'] = self.parseAndNormalize(
                            side_data.get('blue_y'), 50000)
                        side_data['white_point_x'] = self.parseAndNormalize(
                            side_data.get('white_point_x'), 50000)
                        side_data['white_point_y'] = self.parseAndNormalize(
                            side_data.get('white_point_y'), 50000)
                        side_data['min_luminance'] = self.parseAndNormalize(
                            side_data.get('min_luminance'), 10000)
                        side_data['max_luminance'] = self.parseAndNormalize(
                            side_data.get('max_luminance'), 10000)
                        break
            return framedata
        except:
            return framedata
    ...
    def isHDR(self, videostream):
        if len(self.settings.hdr['space']) < 1 and len(self.settings.hdr['transfer']) < 1 and len(self.settings.hdr['primaries']) < 1:
            self.log.debug(
                "No HDR screening parameters defined, returning false [hdr].")
            return False
        ...
        for param in params:
            if param in videostream.color and len(self.settings.hdr.get(param)) > 0 and videostream.color.get(param) not in self.settings.hdr.get(param):
                self.log.debug("Stream is not HDR, color parameter %s does not match %s [hdr-%s]." % (
                    videostream.color.get(param), self.settings.hdr.get(param), self.settings.hdr.get(param)))
                return False
    ...
    def isImageBasedSubtitle(self, inputfile, map):
        ...
        try:
            conv = self.converter.convert(
                None, options, timeout=30, postopts=postopts)
            _, cmds = next(conv)
    ...
    def canBypassConvert(self, inputfile, info, options=None):
        if info.video.codec == "mpeg4" or info.video.codec == "mpeg2video" or "mpeg" in info.video.codec:
        ...
        if self.settings.output_extension == self.parseFile(inputfile)[2]:
            if not self.settings.force_convert and not self.settings.process_same_extensions:
                self.log.info(
                    "Input and output extensions are the same so passing back the original file [process-same-extensions: %s]." % self.settings.process_same_extensions)
                return True
            elif info.format.metadata.get('encoder', '').startswith('sma') and not self.settings.force_convert:
                self.log.info(
                    "Input and output extensions match and the file appears to have already been processed by SMA, enable force-convert to override [force-convert: %s]." % self.settings.force_convert)
                return True
                ...
                self.log.info(
                    "Input and output extensions match, the file appears to copying all streams, and is not reducing the number of streams, enable force-convert to override [bypass-if-copying-all] [force-convert: %s]." % self.settings.force_convert)
                return True
    ...
    def convert(self, options, preopts, postopts, reportProgress=False, progressOutput=None):
        ...
        outputfile, output_dir = self.getOutputFile(
            input_dir, filename, input_extension, self.settings.temp_extension)
        finaloutputfile, _ = self.getOutputFile(
            input_dir, filename, input_extension)
        ...
        if os.path.abspath(inputfile) == os.path.abspath(finaloutputfile):
            self.log.debug(
                "Inputfile and final outputfile are the same, trying to rename inputfile first.")
            try:
                ...
                if self.settings.burn_subtitles:
                    try:
                        if self.raw(os.path.abspath(inputfile)) in (options['video'].get('filter') or ""):
                            self.log.debug(
                                "Renaming inputfile in burnsubtitles filter if its present [burn-subtitles].")
                            options['video']['filter'] = options['video']['filter'].replace(
                                self.raw(os.path.abspath(inputfile)), self.raw(os.path.abspath(og)))
                    except:
                        self.log.exception(
                            "Error trying to rename filter [burn-subtitles].")
                inputfile = og
                options['source'][0] = og
                ...
                while os.path.isfile(finaloutputfile):
                    outputfile, output_dir = self.getOutputFile(
                        input_dir, filename, input_extension, self.settings.temp_extension, number=i)
                    finaloutputfile, _ = self.getOutputFile(
                        input_dir, filename, input_extension, number=i)
                    i += 1
                self.log.debug(
                    "Unable to rename inputfile. Alternatively renaming output file to %s." % outputfile)
        ...
        try:
            conv = self.converter.convert(outputfile, options, timeout=None, preopts=preopts,
                                          postopts=postopts, strip_metadata=self.settings.strip_metadata)
        except:
        ...
        try:
            ...
            try:
                ...
                self.log.exception(
                    "Error restoring original inputfile after exception.")
                return None, inputfile
        except:
            ...
            try:
                ...
                self.log.exception(
                    "Error restoring original inputfile after FFMPEG error.")
                return None, inputfile
        ...
        if outputfile != finaloutputfile:
            self.log.debug(
                "Outputfile and finaloutputfile are different attempting to rename to final extension [temp_extension].")
            try:
                os.rename(outputfile, finaloutputfile)
            except:
                self.log.exception(
                    "Unable to rename output file to its final destination file extension [temp_extension].")
    ...
    def displayProgressBar(self, complete, debug="", width=20, newline=False):
        try:
            ...
            sys.stdout.write('[{0}] {1}% '.format(
                '#' * int(round(complete / divider)) + ' ' * int(round(width - (complete / divider))), complete))
        ...
        if os.path.isfile(inputfile) and self.settings.relocate_moov:
            ...
            try:
                outputfile = inputfile.decode(
                    sys.getfilesystemencoding()) + temp_ext
            except:
        ...
        if self.settings.copyto:
            ...
            for d in self.settings.copyto:
                ...
                try:
                    ...
                    self.log.exception(
                        "First attempt to copy the file has failed.")
                    try:
                        if os.path.exists(inputfile):
                            self.removeFile(inputfile, 0, 0)
                        try:
                            shutil.copy(inputfile.decode(
                                sys.getfilesystemencoding()), d)
                        except:
                            shutil.copy(inputfile, d)
                        self.log.info("%s copied to %s." % (inputfile, d))
                        files.append(os.path.join(
                            d, os.path.split(inputfile)[1]))
                    except:
                        self.log.exception(
                            "Unable to create additional copy of file in %s." % (d))

        if self.settings.moveto:
            self.log.debug("Moveto option is enabled.")
            moveto = os.path.join(
                self.settings.moveto, relativePath) if relativePath else self.settings.moveto
            ...
            try:
                ...
                self.log.exception(
                    "First attempt to move the file has failed.")
                try:
                    if os.path.exists(inputfile):
                        self.removeFile(inputfile, 0, 0)
                    shutil.move(inputfile.decode(
                        sys.getfilesystemencoding()), moveto)
                    self.log.info("%s moved to %s." % (inputfile, moveto))
                    files[0] = os.path.join(
                        moveto, os.path.basename(inputfile))
                except:
                    self.log.exception("Unable to move %s to %s" %
                                       (inputfile, moveto))
    ...
    def removeFile(self, filename, retries=2, delay=10, replacement=None):
        for i in range(retries + 1):
            try:
                ...
                self.log.debug(
                    "Unable to set file permissions before deletion. This is not always required.")
            try:
                ...
                self.log.exception(
                    "Unable to remove or replace file %s." % filename)
                if delay > 0:
                    self.log.debug(
                        "Delaying for %s seconds before retrying." % delay)
                    time.sleep(delay)
